...
#The following code extracts date, ticker and text from the json file in the NASDAQ data folder and creates a csv file with these three columns
...
output_file_name = "sorted_processed_data.csv"
...
with open(json_file_path, 'r') as json_file:
    ...
    for i, line in enumerate(json_file):
        ...
        if len(ticker.split(sep=",")) == 1 and ticker != "":
            ...
            #Get permno of company for fetching data from wrds
            ...
            data_fill.append([date, ticker, text, count_words, permno["permno"].iloc[-1]])

    data = pd.DataFrame(data_fill, columns = ["Date", "Ticker", "Text", "Word Count", "Permno"])
...
with open(output_file_path + output_file_name, "w") as csv_file:
    ...
    writer.writerow(["Date", "Ticker", "Text", "Word Count", "Permno", "Year", "Week"])
